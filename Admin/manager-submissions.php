...
<head>
    ...
    <style>
        ...
        .btn-review  { background:#f39c12; color:#fff; border:none; padding:4px 10px; border-radius:4px; cursor:pointer; font-size:.78em; font-family:'Quicksand',sans-serif; transition:.2s; }
        ...
        .action-done { color:#888; font-size:.8em; font-weight:600; }
    ...
    <main class="main-content">
        ...
        <div class="table-card">
            ...
            <table>
                ...
                <tbody id="subBody"><tr><td colspan="9" class="empty">Φόρτωση...</td></tr></tbody>
            </table>
        </div>

        <div class="table-card">
            <div class="table-card-header">Αξιωματούχοι που δεν υπέβαλαν για το έτος <span id="missYear"><?= $currentYear ?></span></div>
...
function fmt(dt) {
    return dt ? new Date(dt).toLocaleDateString('el-GR', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric',
        hour: '2-digit',
...
function statusLabel(s) {
    return {Pending: 'Εκκρεμεί', Approved: 'Εγκρίθηκε', Rejected: 'Απορρίφθηκε'}[s] || s;
}
...
function renderSubs(subs) {
    document.getElementById('k-total').textContent = subs.length;
    document.getElementById('k-pend').textContent = subs.filter(s => s.status === 'Pending').length;
    document.getElementById('k-appr').textContent = subs.filter(s => s.status === 'Approved').length;
    document.getElementById('k-rej').textContent = subs.filter(s => s.status === 'Rejected').length;

    const tb = document.getElementById('subBody');
    if (!subs.length) {
        tb.innerHTML = '<tr><td colspan="9" class="empty">Δεν βρέθηκαν υποβολές για τα επιλεγμένα κριτήρια</td></tr>';
        return;
    }
    ...
    tb.innerHTML = subs.map(s => `
        <tr data-name="${esc(s.first_name)} ${esc(s.last_name)}">
            ...
            <td>${s.pdf_path ? `<a href="${esc(s.pdf_path)}" target="_blank" rel="noopener">PDF</a>` : '—'}</td>
            ...
            <td class="action-cell">${renderActions(s)}</td>
        </tr>
    `).join('');
...
function renderActions(s) {
    if (s.status !== 'Pending') {
        return '<span class="action-done">Η ενέργεια έχει ήδη ληφθεί</span>';
    }
    ...
    return `
        ...
        <button class="btn-review" onclick="openModal(${s.submission_id}, '${esc(s.notes || '')}')">Σχόλιο</button>
        <button class="btn-del" onclick="deleteSub(${s.submission_id})">Διαγραφή</button>
    `;
}
...
function renderMissing(list) {
    ...
    if (!list.length) {
        w.innerHTML = '<div style="color:#27ae60;text-align:center;padding:20px;font-weight:600">Όλοι οι αξιωματούχοι έχουν υποβάλει για το επιλεγμένο έτος</div>';
        return;
    }
...
function quickStatus(id, status) {
    post({action: 'update_status', submission_id: id, status, notes: ''})
        .then(res => {
            if (res.success) {
                Swal.fire({icon: 'success', title: statusLabel(status), timer: 1000, showConfirmButton: false});
                loadAll();
            } else {
                Swal.fire({icon: 'error', title: 'Σφάλμα', text: res.error});
            }
        });
}

function openModal(id, notes) {
    document.getElementById('mSubId').value = id;
    document.getElementById('mStatus').value = 'Approved';
...
function saveStatus() {
    ...
    post({action: 'update_status', submission_id: id, status, notes})
        .then(res => {
            if (res.success) {
                closeModal();
                Swal.fire({icon: 'success', title: 'Ενημερώθηκε', timer: 1000, showConfirmButton: false});
                loadAll();
            } else {
                Swal.fire({icon: 'error', title: 'Σφάλμα', text: res.error});
            }
...
function deleteSub(id) {
    Swal.fire({
        title: 'Διαγραφή;',
        text: 'Η υποβολή θα διαγραφεί οριστικά.',
        icon: 'warning',
        ...
        if (r.isConfirmed) {
            post({action: 'delete', submission_id: id}).then(res => {
                if (res.success) {
                    Swal.fire({icon: 'success', title: 'Διαγράφηκε', timer: 1000, showConfirmButton: false});
                    loadAll();
                } else {
                    Swal.fire({icon: 'error', title: 'Σφάλμα', text: res.error});
                }
